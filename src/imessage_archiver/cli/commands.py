...
def _run_archive(source_db: Path, bundle_path: Path) -> None:
    ...
    if s.messages_in_icloud_likely and s.cloud_unfetched_attachments > 0:
        ...
        console.print(
            ...
            "attachments exist in iCloud but are not downloaded to this Mac, "
            "so they could not be archived."
        )
        console.print(
            "  Messages in iCloud appears to be enabled. There is no reliable\n"
            "  consumer-facing way on modern macOS to bulk-download all the\n"
            "  cloud-only attachments to this Mac — Apple has progressively\n"
            "  removed the relevant settings and AppleScript hooks. Apple's\n"
            "  privacy data export (privacy.apple.com) also does NOT include\n"
            "  iMessage content (verified 2026-05).\n"
            "\n"
            "  Options that have a real chance of recovering more, in order:\n"
            "  • [bold]Run this archiver on your primary Messages device[/bold]. The\n"
            "    device that's signed into iMessage most often (often an iPhone,\n"
            "    or a different Mac) usually has a much higher local cache hit\n"
            "    rate than a secondary Mac. Run the archiver there; compare counts.\n"
            "  • [bold]Unencrypted iPhone backup via Finder[/bold] → use a tool like\n"
            "    `imessage-exporter` (brew installable, free) to extract messages\n"
            "    + attachments from the backup. Whatever the iPhone has cached\n"
            "    locally will be in the backup.\n"
            "  • Accept the loss — the metadata for ALL messages is captured;\n"
            "    only the binary content of these cloud-only attachments is\n"
            "    missing. The iOS reader will display a 'partial archive' badge.\n"
            "\n"
            "  Manifest fields 'messages_in_icloud_likely' and\n"
            "  'cloud_unfetched_attachment_count' record this for the iOS reader."
        )
